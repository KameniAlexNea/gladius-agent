...
def _make_kickoff_prompt(state: CompetitionState) -> str:
    ...
    if state.iteration == 1:
        return (
            ...
            "1. Read `.claude/agent-memory/team-lead/MEMORY.md` to confirm there is no prior context.\n"
            "2. Delegate to `team-lead` to plan a baseline experiment.\n"
            "3. Execute the full pipeline in order: "
            "data-expert → feature-engineer → ml-engineer → evaluator → validator → memory-keeper.\n"
            "Focus on getting a clean, reproducible baseline score above all else."
        )

    best = (
        f"{state.best_oof_score:.6f}" if state.best_oof_score is not None
        else (f"{state.best_quality_score}/100" if state.best_quality_score is not None else "none yet")
    )
    ...
    return (
        ...
        "1. Read `.claude/agent-memory/team-lead/MEMORY.md` for the full history of what has been tried.\n"
        "2. Delegate to `team-lead` to plan the next experiment using the lessons from MEMORY.md.\n"
        "3. Execute only the pipeline stages that need to run "
        "(check `.claude/EXPERIMENT_STATE.json` — skip agents whose entry is already `\"status\": \"success\"`).\n"
...
async def run_competition(
    ...
    reason = (
        ...
        or ("done_signal" if state.done else "max_iterations")
    )
    logger.info(
