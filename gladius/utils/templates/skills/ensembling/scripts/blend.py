...
def rank_average(oof_preds: dict, test_preds: dict, y_train, metric_fn=roc_auc_score):
    ...
    rank_oof = np.column_stack([rankdata(oof_preds[n]) / n_train for n in names]).mean(
        axis=1
    )
    rank_tst = np.column_stack([rankdata(test_preds[n]) / n_test for n in names]).mean(
        axis=1
    )
...
def save_submission(
    test_df: "pd.DataFrame", preds: np.ndarray, path: str = "submission_ensemble.csv"
):
    sub = (
        test_df[["id"]].copy()
        if "id" in test_df.columns
        else test_df.iloc[:, :1].copy()
    )
