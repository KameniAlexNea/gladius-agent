...
This is a direct evolution of the old n_parallel > 1 implementer mode,
but now each branch is a full functional pipeline rather than a single agent.
"""
...
async def _run_one_branch(
    ...
    """Run a single autonomous mini-team branch in a branch-scoped working dir."""
    # Each branch operates in the same project dir but uses an isolated
    # EXPERIMENT_STATE JSON to avoid collisions.
    branch_state_path = (
...
class AutonomousTopology(BaseTopology):
    ...
    async def run_iteration(
        ...
        *,
        ...
        # Budget: count each branch as one agent call
